...
const getOuputDir = package => path.join(__dirname, "..", "packages", package, "obj");
...
function compileRuntime({ inputFile, inputWasmFile, format, package }) {
  console.log("Building %s", inputFile);

  const OUTPUT_DIRECTORY = getOuputDir(package);
  ...
  const runtimeText = String(fs.readFileSync(path.join(BASE_DIRECTORY, inputFile), {
    encoding : "utf8"
  }));
